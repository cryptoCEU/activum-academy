    ...
    <div className={`flex flex-col h-full bg-act-white border-r border-act-beige1 transition-all duration-300 ${collapsed ? 'w-14' : 'w-72'}`}>
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {courseData.modules.map((mod) => {
          ...
          if (collapsed) {
            return (
              <button key={mod.id} onClick={() => { setCollapsed(false); toggle(mod.id) }}
                className={`w-full flex justify-center py-2.5 transition-colors ${isActiveMod ? 'text-act-burg' : 'text-act-beige3 hover:text-act-black'}`}
          ...
          return (
            <div key={mod.id}>
              <button onClick={() => toggle(mod.id)}
                className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-left transition-colors group ${isActiveMod ? 'bg-act-beige1' : 'hover:bg-act-beige1/50'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-act-beige3 mt-0.5">{modDone}/{mod.lessons.length} lecciones</div>
                </div>
              ...
              {exp && (
                <div className="border-l border-act-beige2 ml-6 animate-slide-in">
                  {mod.lessons.map(lesson => {
                    ...
                    return (
                      <button key={lesson.id} onClick={() => onSelectLesson(mod.id, lesson.id)}
                  ...
                  {/* Quiz row */}
                  <button onClick={() => onSelectQuiz(mod.id)}
